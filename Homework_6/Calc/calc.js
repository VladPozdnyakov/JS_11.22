...
addEventListener("keydown", function addNumericKeyBoardInput(evt) {
  ...
  if (operations.hasOwnProperty(evt.key.toLowerCase()))
    operations[evt.key.toLowerCase()]();
});

function mathOperations(op) {
  // appends math operators
  const possibleOp = ["+", "-", "*", "/"];
  const lastChar = document
    .querySelector("#calc")
    .innerText.charAt(document.querySelector("#calc").innerText.length - 1);
...
function unique(op) {
  ...
  if (!calcArr[calcArr.length - 1].includes(op))
    document.querySelector("#calc").innerText += op;
}
...
function uniqueStr(str) {
  ...
  if (
    calcArea.substr(calcArea.length - str.length, calcArea.length - 1) != str
  ) {
...
function changeSignal() {
  ...
  if (calcArea.textContent.length > 0) {
    signalChanged === false
      ? ((calcArea.textContent = calcArea.textContent.replace(
          calcArr[0],
          "-" + calcArr[0]
        )),
        (signalChanged = true))
...
function displayEquals() {
  // displays the result
  document.querySelector("#results").innerText = Function(
    `'use strict'; return (${document.querySelector("#calc").innerText})`
  )();
}
